...
function hashData(data) {
    ...
    return crypto.createHash('sha256').update(cleanedData.toLowerCase().trim()).digest('hex');
}
    ...
    try {
        await client.query(`
            CREATE TABLE IF NOT EXISTS vendas (
                id SERIAL PRIMARY KEY, chave TEXT UNIQUE NOT NULL, hash TEXT UNIQUE NOT NULL, valor REAL NOT NULL, 
...
async function saveUserClickAssociation(telegramUserId, uniqueClickId) {
    const sql = `INSERT INTO telegram_users (telegram_user_id, unique_click_id, last_activity) VALUES ($1, $2, NOW()) ON CONFLICT (telegram_user_id) DO UPDATE SET unique_click_id = EXCLUDED.unique_click_id, last_activity = NOW();`;
    try {
...
async function limparFrontendUtmsAntigos() {
    ...
    try {
        ...
        if(res.rowCount > 0) console.log(`🧹 Limpeza de UTMs antigos: ${res.rowCount || 0} registros removidos.`);
    } catch (err) { console.error('❌ Erro ao limpar UTMs antigos:', err.message); }
}

// --- ENDPOINTS HTTP ---
app.post('/frontend-utm-data', (req, res) => {
    console.log('🚀 [BACKEND] Dados do frontend recebidos:', req.body);
    if (!req.body.unique_click_id || !req.body.timestamp) { return res.status(400).send('unique_click_id e Timestamp são obrigatórios.'); }
...
app.listen(PORT || 3000, () => {
    ...
    (async () => {
        ...
        if (!TELEGRAM_SESSION) return console.error("❌ ERRO FATAL: TELEGRAM_SESSION não definida.");
        ...
        client.addEventHandler(async (event) => {
            ...
            try {
                ...
                const nomeMatch = texto.match(/Nome\s+Completo[:：]?\s*(.+)/i);
                const emailMatch = texto.match(/E-mail[:：]?\s*(\S+@\S+\.\S+)/i);
                const codigoVendaMatch = texto.match(/Código\s+de\s+Venda[:：]?\s*(.+)/i);
                const plataformaPagamentoRegex = /Plataforma\s+Pagamento[:：]?\s*(.+)/i;
                const metodoPagamentoRegex = /M[ée]todo\s+Pagamento[:：]?\s*(.+)/i;
                
                const nomeDaMensagem = (nomeMatch || [])[1]?.trim().split('|')[0] || "Cliente Desconhecido";
                const emailDaMensagem = (emailMatch || [])[1]?.trim();
                const valorDaMensagem = parseFloat(valorLiquidoMatch[1].replace(/\./g, '').replace(',', '.'));
                const codigoVendaDaMensagem = (codigoVendaMatch || [])[1]?.trim();
                ...
                if (PUSHINPAY_API_TOKEN) {
                    try {
                        ...
                        console.warn(`⚠️  Não foi possível consultar a API da Pushinpay.`);
                    }
                }
                ...
                if (finalCustomerDocument) console.log(`   -> ✅ Documento (CPF/CNPJ) do cliente obtido via API.`);
                else console.log(`   -> ⚠️  Documento do cliente não encontrado.`);
                ...
                // =================================================================
                // <<< INÍCIO DA CORREÇÃO DEFINITIVA PARA O ERRO DA UTMIFY >>>
                // =================================================================
                ...
                if (API_KEY) {
                    ...
                    // 1. Montamos o payload base SEM a chave 'trackingParameters'
                    const utmifyPayload = {
                        orderId: transaction_id, platform, paymentMethod, status: 'paid', createdAt: agoraUtc, approvedDate: agoraUtc,
                        customer: { name: finalCustomerName, email: finalCustomerEmail || "naoinformado@utmify.com", phone: null, document: finalCustomerDocument, ip: matchedFrontendUtms?.ip || '0.0.0.0' },
                        products: [{ id: 'acesso-vip-bundle', name: 'Acesso VIP', planId: '', planName: '', quantity: 1, priceInCents: Math.round(finalValor * 100) }],
                        commission: { totalPriceInCents: Math.round(finalValor * 100), gatewayFeeInCents: 0, userCommissionInCents: Math.round(finalValor * 100), currency: 'BRL' },
                        isTest: false
                    };
                    
                    // 2. Adicionamos a chave 'trackingParameters' APENAS SE houver dados de clique
                    ...
                    // 3. Agora o envio é feito com o payload correto para ambos os cenários
                    try {
                ...
                // =================================================================
                // <<< FIM DA CORREÇÃO DEFINITIVA PARA O ERRO DA UTMIFY >>>
                // =================================================================
                ...
                if (FACEBOOK_PIXEL_ID && FACEBOOK_API_TOKEN) {
                    ...
                    const userData = {
                        ...
                        client_ip_address: matchedFrontendUtms?.ip, client_user_agent: matchedFrontendUtms?.user_agent,
                        fbc: matchedFrontendUtms?.fbc, fbp: matchedFrontendUtms?.fbp,
                    };
                    Object.keys(userData).forEach(key => (!userData[key] || !userData[key][0]) && delete userData[key]);
                    
                    const facebookPayload = { data: [{ event_name: 'Purchase', event_time: message.date, event_id: transaction_id, action_source: 'website', user_data: userData, custom_data: { value: finalValor, currency: 'BRL' } }] };
                ...
                await salvarVenda({
                    chave: gerarChaveUnica({ transaction_id }), hash, valor: finalValor,
                    utm_source: matchedFrontendUtms?.utm_source, utm_medium: matchedFrontendUtms?.utm_medium,
                    utm_campaign: matchedFrontendUtms?.utm_campaign, utm_content: matchedFrontendUtms?.utm_content,
